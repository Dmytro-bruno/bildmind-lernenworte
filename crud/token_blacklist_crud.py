...
class TokenBlacklistCRUD:
    ...
    def get(db: Session, id):
        return db.query(TokenBlacklist).filter(TokenBlacklist.id == id).first()

    @staticmethod
    def get_all(db: Session, skip=0, limit=100):
        return db.query(TokenBlacklist).offset(skip).limit(limit).all()

    @staticmethod
    def create(db: Session, obj_in):
        db_obj = TokenBlacklist(**obj_in.dict())
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        return db_obj
